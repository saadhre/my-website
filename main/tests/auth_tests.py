from main.tests.general_tests import BaseTestCase


class AuthTests(BaseTestCase):
    def test_login(self):
        result = self.app.get('/pl/auth/login')
...
class ResetPasswordTests(BaseTestCase):
    def test_reset_init(self):
        result = self.app.get('/pl/auth/reset-password')
        assert bytes('Rozpocznij resetowanie hasła', 'utf-8') in result.data

        result = self.app.post('/pl/auth/reset-password')
        assert b'To pole jest wymagane.' in result.data

        result = self.app.post('/pl/auth/reset-password', data=dict(
            email='default'
        ))
        assert bytes('Nieprawidłowy adres e-mail.', 'utf-8') in result.data
    ...
    def test_reset_finalize(self):
        ...
        result = self.app.get('/pl/auth/reset-password/wrong-hash', follow_redirects=True)
        assert bytes('Link resetowania hasła już wygasł, spróbuj ponownie', 'utf-8') in result.data

        result = self.app.get(reset_url)
        assert bytes('Zmień swoje hasło', 'utf-8') in result.data

        result = self.app.post(reset_url, follow_redirects=True)
        ...
        result = self.app.post(reset_url, data=dict(
            password='default',
